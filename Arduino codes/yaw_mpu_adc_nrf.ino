// Master program for gloves data acquistion and transmission to serial gateway
// 5 x ADC + MPU YPR data acquitition
// Data stored in data variable separated using /
// Transmitted through nrf24L01+ module
// Arduino Nano + MPU6050 + nRF24L01+ setup
// Analog 0 1 2 3 6 pins used for ADC
...
void setup()
{
 ...
 transmit_data[0] = 0;      // 0 - left glove ; 1 - right glove

 mpu.resetFIFO();
  if (!dmpReady) // if DMP is not ready, exit from program
    return;
  for(int i=0;i<yaw_delay;i++)
  {
    imu_data_collect();
//    Serial.print(i);
//    Serial.print(" - ");
//    Serial.println(ypr_val[0]);
  }
  yaw_initial = ypr_val[0];
}
  ...
  if (!dmpReady) // if DMP is not ready, exit from program
    ...
    for(int i=0;i<5;i++)
    {
      adc_smooth[i] = digitalSmooth(adc_val[i], sensSmoothArray[i]);
        //adc_smooth[i] = adc_val[i];
    }

    transmit_data[1] = ypr_val[0] - yaw_initial + 180;
    for(int i=1;i<3;i++)
    {
      transmit_data[i+1] = ypr_val[i];
    }
    
    for(int i=0;i<5;i++)
    {
      transmit_data[i+4] = adc_smooth[i];
    }
